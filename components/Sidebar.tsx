...
const Sidebar: React.FC<SidebarProps> = ({ profile, signOut, currentPage, onNavigate, isOpen, onClose }) => {
  ...
  const ALL_MENU_ITEMS: MenuItem[] = [
    ...
    { page: 'invoices', label: 'Faturas', icon: <InvoiceIcon /> },
  ...
  const filteredMenuItems = ALL_MENU_ITEMS.filter(item => isAdmin || userPermissions.includes(item.page));
      ...
      <div className="flex flex-col h-full bg-gray-800">
        ...
        <nav className="flex-1 p-4 space-y-2 overflow-y-auto custom-scrollbar">
            {filteredMenuItems.map(item => (
                <NavLink key={item.page} {...item} currentPage={currentPage} onClick={onNavigate} />
            ))}
        </nav>

        <div className="p-4 border-t border-gray-700">
          <button onClick={signOut} className="flex items-center w-full px-4 py-3 text-left text-gray-400 hover:bg-red-900/20 hover:text-red-400 rounded-lg transition-colors">
  ...
  return (
    <>
      ...
      <div className="hidden lg:flex lg:flex-shrink-0 z-20"><div className="w-64 bg-gray-800 border-r border-gray-700">{sidebarContent}</div></div>
    </>
  );
};
...
const PaymentIcon = () => <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3-3V8a3 3 0 003-3h12a3 3 0 003 3v8a3 3 0 00-3 3z"></path></svg>;
